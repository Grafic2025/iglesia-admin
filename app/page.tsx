      ...
      <div style={{ background: '#1E1E1E', padding: '25px', borderRadius: '20px', marginBottom: '30px', border: '1px solid #333' }}>
        ...
        <div style={{ overflowX: 'auto', marginTop: '15px' }}>
          <table style={{ width: '100%', fontSize: '12px', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ color: '#888', textAlign: 'center', borderBottom: '1px solid #333' }}>
                <th style={{ padding: '12px 8px' }}>Fecha</th>
                <th style={{ padding: '12px 8px' }}>Título</th>
                <th style={{ padding: '12px 8px' }}>Mensaje</th>
                <th style={{ padding: '12px 8px' }}>Destinatarios</th>
                <th style={{ padding: '12px 8px' }}>Estado</th>
              </tr>
            </thead>
            <tbody>
              {logsError && <tr><td colSpan={5} style={{ padding: '20px', textAlign: 'center', color: '#ff4444' }}>⚠️ Error: {logsError}</td></tr>}
              {logs.length === 0 && !logsError && <tr><td colSpan={5} style={{ padding: '20px', textAlign: 'center', color: '#555' }}>Sin logs recientes</td></tr>}
              {logs.map(l => (
                <tr key={l.id} style={{ borderBottom: '1px solid #252525', textAlign: 'center' }}>
                  <td style={{ padding: '12px 8px', color: '#aaa', whiteSpace: 'nowrap' }}>{new Date(l.fecha).toLocaleDateString()}</td>
                  <td style={{ padding: '12px 8px', fontWeight: 'bold' }}>{l.titulo}</td>
                  <td style={{ padding: '12px 8px', color: '#ddd', maxWidth: '300px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={l.mensaje}>
                    {l.mensaje}
                  </td>
                  <td style={{ padding: '12px 8px' }}>{l.destinatarios_count} personas</td>
                  <td style={{ padding: '12px 8px', color: l.estado === 'Exitoso' ? '#A8D500' : '#ff4444', fontWeight: 'bold' }}>{l.estado}</td>
                </tr>
              ))}
            </tbody>
          </table>
