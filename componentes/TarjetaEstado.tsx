    ...
    return (
        <div className={`p-4 rounded-xl bg-[#1E1E1E] border border-[#333] transition-all duration-300 flex flex-col justify-between h-full ${isActive ? 'shadow-[0_4px_20px_-5px_rgba(168,213,0,0.1)]' : ''}`}>
            <div className="flex items-start justify-between gap-2 mb-2">
                <span className="text-white/80 text-[10px] md:text-[11px] font-black uppercase tracking-wider leading-tight break-words pt-1" style={{ overflowWrap: 'break-word', wordBreak: 'normal' }}>{label}</span>
            ...
            {trend !== undefined && trend !== null && (
                <div className="flex items-center gap-1 mt-2">
                    ...
                    <span className="text-[10px] text-white/60 font-bold uppercase tracking-tight">vs ayer</span>
                </div>
            )}
        </div>
    );
