    ...
    return (
        <div className={`bg-[#1E1E1E] p-5 rounded-2xl border border-[#333] flex flex-col md:flex-row md:items-center justify-between hover:border-[#00D9FF50] transition-all group gap-4 ${showArchived ? 'opacity-60' : ''}`}>
            <div className="flex items-center gap-4">
                <div className={`shrink-0 w-12 h-12 ${member.es_servidor ? 'bg-[var(--accent)20] text-accent' : 'bg-[#00D9FF15] text-[#00D9FF]'} rounded-full flex items-center justify-center font-bold text-xl overflow-hidden`}>
                    {member.foto_url ? (
                ...
                <div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-xs text-[rgba(255,255,255,0.7)] mt-1">
                        ...
                        {member.es_servidor && <span className="bg-[var(--accent)20] px-2 py-0.5 rounded text-accent font-bold">SERVIDOR</span>}
            ...
            <div className="flex flex-wrap gap-2">
                ...
                <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2 rounded-xl font-bold text-sm transition-all ${member.es_servidor
                        ? 'bg-accent text-black shadow-[0_0_15px_rgba(168,213,0,0.3)]'
