...
interface VistaBotProps {
    supabase: any;
    registrarAuditoria?: (accion: string, detalle: string) => Promise<void>;
}

import { usarBot } from '../../ganchos/usarBot';

const VistaBot = ({ supabase, registrarAuditoria }: VistaBotProps) => {
    const {
        aprendizaje,
        cerebro,
        loading,
        ...
        deleteCerebro,
        openEnseñar
    } = usarBot({ supabase, registrarAuditoria });
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                <div>
                    <h2 className="text-3xl font-black text-white tracking-tighter flex items-center gap-3">
                        <div className="p-2 bg-[var(--accent)15] rounded-xl"><Bot className="text-accent" size={28} /></div>
                ...
                <div className="flex bg-[#1E1E1E] p-1.5 rounded-2xl border border-[#333]">
                    <button
                        ...
                        className={`px-6 py-2.5 rounded-xl text-xs font-black transition-all flex items-center gap-2 ${activeTab === 'aprendizaje' ? 'bg-accent text-black shadow-lg shadow-[var(--accent)15]' : 'text-[rgba(255,255,255,0.7)] hover:text-white'}`}
                    >
                        <Sparkles size={14} /> LO QUE NO SÉ ({aprendizaje.length})
                    </button>
                    <button
                        onClick={() => setActiveTab('cerebro')}
                        className={`px-6 py-2.5 rounded-xl text-xs font-black transition-all flex items-center gap-2 ${activeTab === 'cerebro' ? 'bg-accent text-black shadow-lg shadow-[var(--accent)15]' : 'text-[rgba(255,255,255,0.7)] hover:text-white'}`}
                    >
                ...
                <div className="grid grid-cols-1 gap-6">
                    {activeTab === 'aprendizaje' ? (
                        <div className="bg-[#1E1E1E] p-8 rounded-3xl border border-[#333] shadow-xl">
                            <div className="flex items-center justify-between mb-8">
                                <h3 className="text-white text-xl font-bold flex items-center gap-3">
                                    <History className="text-accent" size={22} /> Consultas pendientes de aprendizaje
                                </h3>
                            </div>
                                        ...
                                        <div key={a.id} className="bg-[#151515] p-5 rounded-2xl border border-[#252525] hover:border-[var(--accent)50] transition-all flex items-center justify-between group">
                                            ...
                                            <div className="flex items-center gap-3">
                                                <button
                                                    ...
                                                    className="bg-accent text-black px-5 py-2.5 rounded-xl font-black text-[10px] hover:scale-105 transition-all flex items-center gap-2 shadow-lg shadow-[var(--accent)15]"
                                                >
                        ...
                        <div className="bg-[#1E1E1E] p-8 rounded-3xl border border-[#333] shadow-xl">
                            <div className="flex items-center justify-between mb-8">
                                <h3 className="text-white text-xl font-bold flex items-center gap-3">
                                    <CheckCircle2 className="text-accent" size={22} /> Conocimiento adquirido
                                </h3>
                                <button
                                    onClick={() => { setCurrentKnowledge(null); setPalabrasClave(''); setRespuesta(''); setShowModal(true); }}
                                    className="bg-[#252525] text-white px-5 py-2.5 rounded-xl font-black text-[10px] border border-[#333] hover:bg-accent hover:text-black transition-all"
                                >
                                    + AGREGAR MANUALMENTE
                                </button>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {cerebro.map((c) => (
                                    <div key={c.id} className="bg-[#151515] p-6 rounded-2xl border border-[#252525] hover:border-[var(--accent)30] transition-all relative group">
                                        <div className="mb-4">
                                            <label className="text-[9px] font-black text-accent uppercase tracking-widest block mb-1">Palabras Clave</label>
                        ...
                        <div className="p-8 space-y-6">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full bg-[#222] border border-[#333] rounded-2xl px-5 py-4 text-white outline-none focus:border-accent transition-all"
                                />
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    rows={5}
                                    className="w-full bg-[#222] border border-[#333] rounded-2xl px-5 py-4 text-white outline-none focus:border-accent transition-all resize-none"
                                />
                            </div>
                            ...
                            <div className="flex gap-4 pt-4">
                                ...
                                <button
                                    ...
                                    className="flex-3 py-4 bg-accent text-black font-black text-xs rounded-2xl shadow-xl shadow-[var(--accent)20] hover:scale-[1.02] active:scale-[0.98] transition-all"
                                >
