...
interface VistaConfigAgendaProps {
    supabase: any;
...
import { usarConfigAgenda } from '../../ganchos/usarConfigAgenda';

const VistaConfigAgenda = ({ supabase, horariosDisponibles, fetchHorarios, registrarAuditoria }: VistaConfigAgendaProps) => {
    const {
        newHorario,
        setNewHorario,
        handleAdd,
        handleRemove
    } = usarConfigAgenda({ supabase, horariosDisponibles, fetchHorarios, registrarAuditoria });
                ...
                <div className="space-y-4">
                    <div className="flex gap-3">
                        <input
                            ...
                            className="flex-1 bg-[#252525] border border-[#333] rounded-2xl px-6 py-4 text-white outline-none focus:border-accent transition-all [color-scheme:dark]"
                        />
                        <button
                            onClick={handleAdd}
                            className="bg-accent text-black font-black px-8 py-4 rounded-2xl hover:shadow-[0_0_20px_rgba(168,213,0,0.4)] transition-all active:scale-95 flex items-center gap-2"
                        >
...
export default VistaConfigAgenda;
