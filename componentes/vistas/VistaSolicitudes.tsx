...
interface VistaSolicitudesProps {
    bautismos: any[];
    ayuda: any[];
    miembros: any[];
}

const VistaSolicitudes: React.FC<VistaSolicitudesProps> = ({ bautismos, ayuda, miembros }) => {
    return (
                                ...
                                return (
                                    <div key={a.id} className="bg-[#252525] p-5 rounded-2xl border border-[#333] hover:border-red-500/30 transition-all">
                                        <div className="flex justify-between items-start mb-4">
                                            <div className="text-white text-lg font-bold">{m ? `${m.nombre} ${m.apellido}` : a.miembro_id}</div>
                                            <a
