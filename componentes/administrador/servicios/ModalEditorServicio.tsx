...
interface ModalEditorServicioProps {
...
const ModalEditorServicio: React.FC<ModalEditorServicioProps> = ({
    selectedSchedule,
    fecha, setFecha,
    horario, setHorario,
    notas, setNotas,
            ...
            <div className="bg-[#151515] w-full max-w-5xl h-[90vh] rounded-3xl border border-[#333] shadow-2xl overflow-hidden flex flex-col text-left">
                        ...
                        <div className="flex gap-3 ml-4">
                            <div className="flex flex-col">
                                ...
                                <input
                                    ...
                                    className="bg-[#222] border border-[#333] rounded-xl px-4 py-2 text-white text-sm outline-none focus:border-accent transition-all cursor-pointer [color-scheme:dark]"
                                />
                            </div>
                            <div className="flex flex-col">
                                ...
                                <div className="flex flex-wrap gap-2 items-center">
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`px-3 py-2 rounded-xl text-xs font-bold transition-all border ${selected
                                                    ? 'bg-accent text-black border-transparent shadow-[0_0_10px_rgba(168,213,0,0.3)]'
                                    ...
                                    <input
                                        ...
                                        className="bg-[#222] border border-[#333] rounded-xl px-3 py-2 text-white text-xs outline-none focus:border-accent transition-all w-28 cursor-pointer [color-scheme:dark]"
                                    />
                                    <button
                                        onClick={() => {
                                        ...
                                        className="bg-[#333] text-accent px-3 py-2 rounded-xl text-xs font-bold hover:bg-accent hover:text-black transition-all border border-[#444]"
                                    >
                                        + Agregar
                                    </button>
                                </div>
                                {horario && (
                                    <div className="flex flex-wrap gap-1.5 mt-2">
                                        {horario.split(', ').filter(Boolean).map(h => (
                                            <span key={h} className="flex items-center gap-1 bg-[var(--accent)15] text-accent text-[10px] font-bold px-2.5 py-1 rounded-lg border border-[var(--accent)30]">
                                                🕐 {h}
                                                <button
                                                    onClick={() => {
                                                    ...
                                                    className="text-accent hover:text-red-400 ml-1"
                                                >×</button>
                                            </span>
                                        ))}
                                    </div>
                ...
                {!selectedSchedule && assignedStaff.length === 0 && (
                    <div className="bg-[var(--accent)10] p-4 flex items-center justify-between border-b border-[var(--accent)20]">
                        <div className="flex items-center gap-3">
                            <div className="bg-accent text-black p-2 rounded-lg"><CheckCircle2 size={16} /></div>
                            <p className="text-accent text-xs font-bold uppercase tracking-widest">¿Quieres cargar una estructura predefinida?</p>
                        </div>
                        <button
                            onClick={onShowTemplatePicker}
                            className="bg-accent text-black px-4 py-1.5 rounded-lg text-[10px] font-black hover:scale-105 transition-all"
                        >
                            USAR PLANTILLA
                        </button>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 lg:grid-cols-3 gap-8 custom-scrollbar">
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-1 gap-2">
                                ...
                                <button
                                    ...
                                    className="p-4 border-2 border-dashed border-[#333] rounded-xl text-[rgba(255,255,255,0.5)] hover:border-accent hover:text-accent transition-all flex items-center justify-center gap-2 font-bold text-xs"
                                >
                    ...
                    <div className="space-y-8">
                        ...
                        <div>
                            <h4 className="text-accent text-[10px] font-black uppercase mb-4 tracking-[0.2em]">Notas del Director</h4>
                            <textarea
                                value={notas}
                                onChange={e => setNotas(e.target.value)}
                                placeholder="Indicaciones generales para el equipo..."
                                className="w-full bg-[#1A1A1A] border border-[#333] rounded-2xl p-4 text-white text-sm h-24 outline-none focus:border-accent resize-none mb-4"
                            />
                        </div>
                    </div>
                </div>
                ...
                <div className="p-6 border-t border-[#333] bg-[#1A1A1A] flex justify-between gap-4">
                    ...
                    <div className="flex gap-4 items-center">
                        ...
                        <button onClick={onSave} className="px-10 py-3 bg-accent text-black font-black rounded-xl hover:shadow-[0_0_20px_rgba(168,213,0,0.5)] transition-all flex items-center gap-2 uppercase tracking-widest text-sm"><Save size={18} /> Guardar Plan</button>
                    </div>
                </div>
            </div>
        </div>
...
export default ModalEditorServicio;
