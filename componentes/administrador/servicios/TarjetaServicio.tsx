...
const TarjetaServicio: React.FC<TarjetaServicioProps> = ({ service, onEdit, onDelete, onNotify, onToggleChat, onExport, searchQuery }) => {
    return (
        <div className="bg-[#1E1E1E] p-6 rounded-2xl border border-[#333] hover:border-[var(--accent)50] transition-all group relative text-left">
            ...
            <p className="text-accent font-bold text-xs mb-3">{service.horario}</p>
                ...
                if (matches.length > 0) {
                    return (
                        <div className="mt-3 p-2 bg-accent/10 rounded-lg border border-accent/20">
                            <p className="text-accent text-[10px] font-bold mb-1 uppercase">Resultado de búsqueda:</p>
                            {matches.map((m, i) => (
            ...
            <div className="flex gap-2 mt-4">
                <button
                    ...
                    className="flex-1 py-2 bg-[#252525] text-white text-[10px] font-bold rounded-lg border border-[#333] hover:bg-accent hover:text-black transition-all uppercase"
                >
                    <Plus size={12} className="inline mr-1" /> Editar
                </button>
                <button
                    onClick={() => onNotify(service)}
                    className="p-2 bg-accent/10 text-accent rounded-lg border border-accent/20 hover:bg-accent hover:text-black transition-all"
                ...
                <button
                    ...
                    className={`p-2 rounded-lg border transition-all ${service.chat_activo ? 'bg-accent text-black border-accent' : 'bg-transparent text-[rgba(255,255,255,0.7)] border-[#333] hover:border-accent hover:text-accent'}`}
