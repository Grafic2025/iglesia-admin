...
interface ModalSelectorCancionProps {
    allSongs: any[];
...
const ModalSelectorCancion: React.FC<ModalSelectorCancionProps> = ({
    allSongs,
    selectedSongIds,
    songSearch,
    setSongSearch,
            ...
            <div className="bg-[#1A1A1A] w-full max-w-lg rounded-3xl border border-[#333] p-6 max-h-[80vh] flex flex-col">
                ...
                <div className="relative mb-4">
                    ...
                    <input
                        ...
                        className="w-full bg-[#222] border border-[#333] rounded-2xl py-3 pl-11 pr-4 text-white text-xs outline-none focus:border-accent transition-all"
                    />
                </div>
                <div className="overflow-y-auto flex-1 space-y-2 pr-2 custom-scrollbar">
                    {allSongs.filter(s =>
                        ...
                        <button
                            ...
                            className={`w-full flex items-center justify-between p-4 rounded-2xl border transition-all text-left ${selectedSongIds.includes(s.id) ? 'bg-accent border-transparent text-black' : 'bg-[#222] border-[#333] text-white hover:border-accent'}`}
                        >
                ...
                <button
                    onClick={onClose}
                    className="w-full py-4 bg-accent text-black font-black rounded-xl mt-6 uppercase tracking-widest text-xs"
                >
                    Listo
                </button>
            </div>
...
export default ModalSelectorCancion;
