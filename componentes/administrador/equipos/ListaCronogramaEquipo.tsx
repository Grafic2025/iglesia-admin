...
interface ListaCronogramaEquipoProps {
    selectedDateSchedule: any;
    upcomingSchedules: any[];
    setSelectedDateSchedule: (s: any) => void;
    setShowModalComposicionEquipo: (b: boolean) => void;
    setActiveTab?: (t: string) => void;
    miembros: any[];
}

const ListaCronogramaEquipo: React.FC<ListaCronogramaEquipoProps> = ({
        ...
        <div className="lg:col-span-2 bg-[#1E1E1E] rounded-3xl border border-[#333] overflow-hidden flex flex-col">
            ...
            <div className="p-6 space-y-4">
                {filteredList.length === 0 ? (
                    ...
                    filteredList.map(s => (
                        <div key={s.id} className="bg-[#252525] p-5 rounded-2xl border border-[#333] hover:border-[var(--accent)50] transition-all">
                                ...
                                <div className="flex-1 px-4">
                                    <div className="flex -space-x-2 overflow-hidden mb-1">
                                        {(s.equipo_ids || []).slice(0, 10).map((staff: any, idx: number) => {
                                            const memberWithPhoto = miembros.find(m => m.id === staff.miembro_id);
                                            const foto = memberWithPhoto?.foto_url || staff.foto_url;

                                            return (
                                                <div key={idx} className="inline-block h-8 w-8 rounded-full ring-2 ring-[#252525] bg-[#333] overflow-hidden flex items-center justify-center text-[10px] text-white border border-[var(--accent)30]" title={`${staff.nombre} (${staff.rol})`}>
                                                    {foto ? (
...
export default ListaCronogramaEquipo;
