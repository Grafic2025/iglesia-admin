...
interface ModalAsignarMiembroProps {
    selectedTeam: any;
    assignRole: string;
...
const ModalAsignarMiembro: React.FC<ModalAsignarMiembroProps> = ({
    selectedTeam,
    assignRole,
    setAssignRole,
    searchQuery,
                ...
                <div className="p-6 space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            className="w-full bg-[#222] border border-[#333] rounded-xl px-4 py-3 text-white outline-none focus:border-accent mb-3"
                        />
                        <div className="space-y-4 mb-2 max-h-[160px] overflow-y-auto pr-2 custom-scrollbar border-b border-[#333] pb-4">
                            {roleCategories.map(cat => (
                                <div key={cat.name}>
                                    <p className="text-[9px] text-accent font-black uppercase mb-2 tracking-wider opacity-70">{cat.name}</p>
                                    <div className="flex flex-wrap gap-1.5 mb-2">
                                        {cat.roles.map((r: string) => (
                                            <button
                                                key={r}
                                                onClick={() => setAssignRole(r)}
                                                className={`text-[9px] px-2.5 py-1.5 rounded-lg border font-bold transition-all ${assignRole === r ? 'bg-accent text-black border-transparent' : 'bg-[#222] text-[rgba(255,255,255,0.7)] border-[#333] hover:border-[var(--accent)50]'
                                                    }`}
                                            >{r}</button>
                                        ))}
                                    </div>
                    ...
                    <div>
                        ...
                        <div className="relative mb-4">
                            ...
                            <input
                                ...
                                className="w-full bg-[#222] border border-[#333] rounded-xl pl-10 pr-4 py-3 text-white outline-none focus:border-accent"
                            />
                        </div>
                        <div className="max-h-60 overflow-y-auto space-y-2 pr-2">
                            {filteredMembers.map(m => (
                                <button
                                    ...
                                    className="w-full flex items-center gap-3 p-3 bg-[#252525] hover:bg-accent hover:text-black rounded-xl transition-all group group"
                                >
...
export default ModalAsignarMiembro;
