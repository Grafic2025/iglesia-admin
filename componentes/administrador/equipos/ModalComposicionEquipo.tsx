...
interface ModalComposicionEquipoProps {
...
const ModalComposicionEquipo: React.FC<ModalComposicionEquipoProps> = ({
            ...
            <div className="bg-[#151515] w-full max-w-2xl rounded-3xl border border-[#333] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
                <div className="p-6 border-b border-[#333] flex items-center justify-between bg-[#1A1A1A]">
                    <div>
                        <h3 className="text-white font-black text-[14px] uppercase tracking-widest flex items-center gap-3">
                            <Users2 className="text-accent" size={18} /> Composición del Equipo
                        </h3>
                ...
                <div className="p-8 max-h-[70vh] overflow-y-auto space-y-8 custom-scrollbar">
                    ...
                    {categories.map(catName => {
                        const miembrosInCat = staff.reduce((acc: any[], s: any) => {
                        ...
                        if (miembrosInCat.length === 0) return null;

                        return (
                            <div key={catName} className="space-y-4">
                                <div className="flex items-center gap-4">
                                    <h4 className="text-accent text-[10px] font-black uppercase tracking-[0.3em]">{catName}</h4>
                                        ...
                                        <div key={`${s.miembro_id}-${s.specificRol}-${idx}`} className={`flex flex-col p-4 rounded-2xl border transition-all ${s.estado === 'rechazado' ? 'bg-red-500/10 border-red-500/50' : 'bg-[#1E1E1E] border-[#333]'}`}>
                                            <div className="flex items-center gap-4">
                                                <div className="w-12 h-12 rounded-full bg-[#111] border border-[#333] overflow-hidden flex items-center justify-center relative">
                                                    {(() => {
                                                        const m = miembros.find(mem => mem.id === s.miembro_id);
                                                    ...
                                                    <div className={`absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full border-2 border-[#1E1E1E] ${s.estado === 'confirmado' ? 'bg-accent' : s.estado === 'rechazado' ? 'bg-red-500' : 'bg-yellow-500'}`} />
                                                </div>
                                                <div className="flex-1">
                                                    ...
                                                    <p className="text-accent text-[10px] font-black uppercase tracking-wider">{s.specificRol}</p>
                                                </div>
                                            </div>
                ...
                <div className="p-6 border-t border-[#333] bg-[#1A1A1A] flex justify-end">
                    <button
                        onClick={onClose}
                        className="px-8 py-3 bg-accent text-black font-black rounded-xl hover:shadow-[0_0_20px_rgba(168,213,0,0.4)] transition-all uppercase tracking-widest text-xs"
                    >
                        Entendido
                    </button>
                </div>
...
export default ModalComposicionEquipo;
